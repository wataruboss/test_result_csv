...
class new_csv:  #numpyでファイルを読み書きしてみる
    ...
    def output_txt(self):
        for file_name, file_path in self.file_dict.items():
            ...
            ########################################################
            # 値の計算 PlasticStrain_guage_average, TrueStress, TrueStrain
            ########################################################
            row, col = np.shape(input_arr)

            strain_ave = np.reshape((input_arr[:, 5] + input_arr[:, 6]) * 10 / 2, (row, 1))

            n_stress = np.reshape(input_arr[:, 1] / (self.specimen['b'] * self.specimen['h']), (row, 1))

            t_strain = np.reshape((strain_ave / 1000000), (row, 1))

            t_stress = np.reshape(n_stress * np.exp(t_strain), (row, 1))

            output_arr = np.hstack((input_arr, strain_ave, n_stress, t_stress, t_strain))
